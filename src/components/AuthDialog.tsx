  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "signup") {
        ...
        toast.success("Account created — welcome to MoojYatra! 🙏");
        close();
      } else {
      ...
      toast.error(msg[err.code] || err.message || "Authentication failed");
    } finally {
      setLoading(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {mode === "signin"
              ? "Sign in to track points, badges and visits"
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <Button
            ...
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-pink-blue text-white font-bold"
          >
            {loading ? (
              <Loader2 className="w-4 h-4 animate-spin" />
            ) : mode === "signin" ? (
